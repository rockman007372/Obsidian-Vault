...
Stream operations are either _intermediate_ or _terminal_. While terminal operations return a result of a certain type, intermediate operations return the stream itself so you can chain multiple method calls in a row. 
...
We can increase performance with parallel stream. All you have to do is change `stream()` to `parallelStream()`.

However, there are many caveats regarding parallel streams.
