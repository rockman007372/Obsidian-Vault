...
Motivation:
- **Speed:** Stores intermediate results in memory => suitable for **iterative algorithms** / **interactive data analysis** (less disk I/O).
...
- **Fault Tolerance:** rebuild lost data due to node failures by using the **lineage information** to recompute only the lost data partitions instead of re-running the entire job.
